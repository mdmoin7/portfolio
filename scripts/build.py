...
COPY_DIRS = ["assets", "about", "training", "consulting", "engineering", "contact"]
...
PAGE_SEO = {
    "": ("Mohammad Moin — Independent Software Engineering Consultant & Corporate Trainer", "Mohammad Moin is an independent software engineering consultant and corporate technology trainer based in Bengaluru, India, specializing in frontend architecture, Angular, React, React Native and Terraform/Azure, with freelance consulting and corporate training engagements.", ["Mohammad Moin", "software engineering consultant", "corporate technology trainer", "frontend architect", "Angular consultant", "React consultant", "React Native", "Terraform Azure", "frontend architecture"]),
    ...
    "contact": ("Connect with Mohammad Moin — Digital Professional Card", "Connect with Mohammad Moin and save his professional details, portfolio, LinkedIn and GitHub from a digital professional card.", ["Mohammad Moin", "connect", "digital professional card", "software engineering consultant", "corporate technology trainer", "frontend architect"]),
...
AUTHORITY = {
    "training": {"title": "About Mohammad Moin's corporate training practice", "body": "Mohammad Moin delivers corporate technology training and engineering-team upskilling programs grounded in production software engineering. Training is designed for practical adoption: guided theory is combined with implementation exercises, proof-of-concept work, architecture discussions, debugging, performance analysis and code-quality practices.", "facts": ["15,000+ engineers trained across 350+ sessions", "Typical cohorts of 20–50 engineers", "Repeat engagements with returning corporate clients", "Curricula spanning React, Angular, Node.js, React Native, TypeScript, Nx and modern engineering practices"], "proof": "Training clients represented in the professional portfolio include IBM, Amazon, Walmart, SAP and Dell."},
    "consulting": {"title": "About Mohammad Moin's consulting practice", "body": "Mohammad Moin works as an independent software engineering consultant on architecture, modernization and production delivery problems across web, mobile and enterprise applications. Engagements can begin with an architecture review or proof of concept and extend through implementation, migration planning, delivery support and engineering enablement.", "facts": ["Frontend architecture across Angular and React", "Full-stack web and mobile application engineering", "Nx monorepos, shared libraries and platform architecture", "Stack migration, technical POCs and modernization", "Engineering mentoring, debugging and delivery enablement"], "proof": "A recent consulting engagement with JLL involved re-architecting legacy AEM/Java web components to React within a 10–15 engineer team spanning multiple countries, including work around a shared SolidJS design system translated into React and Angular."},
}
...
.authority-section .wrap{max-width:1160px}.authority-section .kicker{display:inline-flex;margin-bottom:8px}.authority-section h2{display:block;margin:0 0 10px;font:600 24px/1.2 var(--display);color:var(--navy);letter-spacing:-.015em}.authority-section .intro{max-width:900px;margin:0;color:var(--muted);font-size:13px;line-height:1.7}.authority-facts{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px 18px;margin:17px 0 0;padding:0;list-style:none}.authority-facts li{position:relative;padding:9px 11px 9px 25px;border:1px solid var(--line);border-radius:8px;background:#fff;color:var(--ink);font-size:11px;line-height:1.5}.authority-facts li::before{content:"";position:absolute;left:11px;top:15px;width:6px;height:6px;border-radius:50%;background:var(--blue);box-shadow:0 0 0 3px var(--blue-soft)}.authority-proof{margin:14px 0 0;padding:11px 13px;border-left:2px solid var(--blue);background:rgba(238,243,255,.55);color:var(--muted);font-size:11px;line-height:1.6}.authority-proof strong{color:var(--navy);font-weight:800}.related-pages{max-width:1160px;margin:0 auto;padding:24px 0 6px}.related-pages h2{display:block;margin:0 0 5px;font:600 21px/1.25 var(--display);color:var(--navy)}.related-pages p{margin:0 0 11px;color:var(--muted);font-size:11px}.related-pages nav{height:auto;position:static;display:flex;flex-wrap:wrap;gap:7px;background:transparent;border:0;backdrop-filter:none}.related-pages nav a{display:inline-flex;align-items:center;padding:7px 10px;border:1px solid #cfdcf1;border-radius:7px;background:var(--blue-soft);color:var(--blue-deep);text-decoration:none;font:800 10px/1.2 var(--mono)}.related-pages nav a:hover{border-color:#aebfe0;background:#e7eeff}@media(max-width:800px){.authority-section{padding:24px 0}.authority-section h2{font-size:21px}.authority-section .intro{font-size:12px}.authority-facts{grid-template-columns:1fr}.related-pages{padding-top:20px}.related-pages h2{font-size:19px}}
"""
...
def normalize_home_navigation():
    path = DIST / "index.html"
    if not path.exists(): return
    source = path.read_text(encoding="utf-8")
    # Keep the homepage Contact anchor for the contact section and expose the digital card as Connect.
    source = re.sub(r'<a href="contact/">(?:06\.|07\.)?\s*Connect</a>', '', source)
    source = source.replace('<a href="#contact">06. Contact</a>', '<a href="#contact">06. Contact</a><a href="contact/">07. Connect</a>', 1)
    path.write_text(source, encoding="utf-8")

def build_schema(key, title, description, keywords, canonical):
    graph = [PERSON, {"@type": ["WebPage", "ProfilePage"] if key == "about" else ["WebPage"], "@id": f"{canonical}#webpage", "url": canonical, "name": title, "description": description, "author": {"@id": PERSON_ID}, "about": {"@id": PERSON_ID}, "mainEntity": {"@id": PERSON_ID}, "keywords": keywords, "inLanguage": "en"}, {"@type": "BreadcrumbList", "@id": f"{canonical}#breadcrumb", "itemListElement": [{"@type": "ListItem", "position": 1, "name": "Mohammad Moin", "item": SITE_URL}, {"@type": "ListItem", "position": 2, "name": title.split(" — ")[0], "item": canonical}]}]
    return json.dumps({"@context": "https://schema.org", "@graph": graph}, ensure_ascii=False, indent=2)

def inject_identity_seo():
    generated_marker = '<!-- portfolio-generated-identity-seo -->'
    generated_pattern = re.compile(r'\s*<!-- portfolio-generated-identity-seo -->.*?<!-- /portfolio-generated-identity-seo -->\s*', re.DOTALL)
    for path in DIST.rglob("*.html"):
        key = page_key(path)
        ...
        additions = f'''{generated_marker}\n<link rel="canonical" href="{canonical}"><meta name="author" content="Mohammad Moin"><meta name="keywords" content="{', '.join(keywords)}"><meta name="robots" content="index, follow, max-image-preview:large, max-snippet:-1, max-video-preview:-1"><meta property="og:type" content="{'profile' if key == 'about' else 'website'}"><meta property="og:title" content="{title}"><meta property="og:description" content="{description}"><meta property="og:url" content="{canonical}"><meta property="og:site_name" content="Mohammad Moin"><meta property="og:image" content="{PROFILE_IMAGE}"><meta name="twitter:card" content="summary_large_image"><meta name="twitter:title" content="{title}"><meta name="twitter:description" content="{description}"><meta name="twitter:image" content="{PROFILE_IMAGE}"><script type="application/ld+json">{schema}</script>\n<!-- /portfolio-generated-identity-seo -->'''
...
def inject_authority_content():
    for key, data in AUTHORITY.items():
        ...
        block = f'''\n<section class="authority-section" data-authority-content="true" aria-labelledby="authority-title"><div class="wrap"><span class="kicker">Professional context</span><h2 id="authority-title">{data["title"]}</h2><p class="intro">{data["body"]}</p><ul class="authority-facts">{facts}</ul><p class="authority-proof"><strong>Selected proof:</strong> {data["proof"]}</p></div></section>\n'''
        source = source.replace("</main>", block + "</main>", 1)
        path.write_text(source, encoding="utf-8")

def inject_related_links():
    for path in DIST.rglob("*.html"):
        key = page_key(path); links = RELATED.get(key)
        if not links: continue
        source = path.read_text(encoding="utf-8")
        if 'data-related-links="true"' in source: continue
        items = " ".join(f'<a href="{href}">{label}</a>' for label, href in links)
        block = f'''\n<section class="related-pages" data-related-links="true" aria-labelledby="related-pages-title"><h2 id="related-pages-title">Explore more from Mohammad Moin</h2><p>Related engineering, consulting and training work:</p><nav aria-label="Related pages">{items}</nav></section>\n'''
        source = source.replace("</main>", block + "</main>", 1)
        path.write_text(source, encoding="utf-8")
...
def rewrite_cdn_assets():
    ...
    for path in DIST.rglob("*.html"):
        ...
        if re.search(r'(?<![A-Za-z0-9_:/.-])assets/profile\.webp', updated): updated = re.sub(r'(?<![A-Za-z0-9_:/.-])assets/profile\.webp', cdn, updated)
        if updated != source: path.write_text(updated, encoding="utf-8")

def minify_assets():
    css = DIST / "styles.css"; css.write_text(rcssmin.cssmin(css.read_text(encoding="utf-8")), encoding="utf-8")
    js = DIST / "main.js"; js.write_text(rjsmin.jsmin(js.read_text(encoding="utf-8")), encoding="utf-8")
...
def validate():
    required = [DIST / "index.html", DIST / "styles.css", DIST / "main.js", DIST / "favicon.svg", DIST / "assets" / "profile.webp", DIST / "about" / "index.html", DIST / "contact" / "index.html", DIST / "contact" / "contact-qr.svg"]
    ...
    for path in html_files:
        content = path.read_text(encoding="utf-8"); key = page_key(path)
        ...
        if key not in PAGE_SEO: raise SystemExit(f"Build failed; PAGE_SEO entry missing for {path.relative_to(DIST)}")
        canonical = f"{SITE_URL}{key}/"
        ...
        if f'<link rel="canonical" href="{canonical}">' not in content: raise SystemExit(f"Build failed; canonical metadata missing in {path.relative_to(DIST)}")
    ...
    if home.count('href="#contact"') != 1 or home.count('href="contact/"') != 1: raise SystemExit("Build failed; expected one Contact section link and one Connect card link in homepage navigation.")
    print(f"Built {len(html_files)} HTML pages")

if __name__ == "__main__":
    copy_site(); inject_favicon(); normalize_home_navigation(); inject_identity_seo(); inject_authority_content(); inject_related_links(); inject_authority_styles(); rewrite_cdn_assets(); minify_assets(); minify_html(); validate()
