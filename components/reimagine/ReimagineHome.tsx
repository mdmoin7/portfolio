  ...
  return (
    <div className={`reimagine ${theme === "light" ? "is-light" : "is-dark"}`}>
      <aside className="spatial-nav" aria-label="Portfolio navigation">
        ...
        <nav>{["THESIS", "SYSTEM", "CAPABILITY", "TECHNOLOGY", "WORK", "TRAINING", "LABS", "ABOUT", "CONTACT"].map((item) => <a key={item} href={`#${item.toLowerCase()}`} className="spatial-link"><span>{item}</span></a>)}</nav>
      ...
      <main>
        <section className="hero-v2" id="thesis">
          ...
          <div className="hero-copy"><p className="eyebrow mono">ENGINEERING × PEOPLE × AI</p><h1>Build people.<br />Solve problems.<br /><em>Innovate.</em></h1><p className="hero-lede">Independent software engineering consultant and corporate technology trainer. I help teams turn complex technology into production systems — and develop the people who can own them.</p><div className="hero-actions"><a href="#work" className="primary-action">Explore the work <Arrow /></a><a href="mailto:mohammad.nicoll@gmail.com" className="text-action">Start a conversation <Arrow /></a></div></div>
          <div className="hero-signature mono">PEOPLE → PROBLEM → DESIGN<br />→ BUILD → IMPACT</div><div className="hero-scroll mono">ENTER THE SYSTEM <span>↓</span></div>
        </section>

        <section className="system-section" id="system"><div className="section-marker mono">01 / SYSTEM</div><div className="system-intro"><p className="eyebrow mono">HOW I THINK</p><h2>Technology matters.<br />Capability makes it <em>last.</em></h2></div>
        ...
        <section className="capability-section" id="capability"><div className="section-marker mono">02 / CAPABILITY</div><div className="capability-copy"><p className="eyebrow mono">WHAT I DO</p><h2>Engineering depth.<br /><em>Human outcomes.</em></h2></div><div className="capability-grid">
          ...
          <article><span className="card-num mono">03</span><h3>Curriculum</h3><p>Technical learning systems that translate real engineering practice into structured, production-oriented programs.</p><a href="#training">See the model <Arrow /></a></article>
        </div></section>

        <section className="technology-section" id="technology" aria-labelledby="technology-title"><div className="section-marker mono">03 / TECHNOLOGY</div><div className="tech-copy"><p className="eyebrow mono">ENGINEERING LANDSCAPE</p><h2>Tools are nodes.<br />Capability is the <em>network.</em></h2></div><div className="constellation" aria-label="Technology constellation">{tech.map((item, i) => <button key={item} className={`tech-node t${i + 1}`} onClick={() => setActive(i % 5)}>{item}</button>)}<div className="constellation-core">ENGINEERING<br /><small>+ AI / RAG</small></div></div></section>

        <section className="work-section" id="work"><div className="section-marker mono">04 / WORK</div><div className="work-head"><div><p className="eyebrow mono">SELECTED SYSTEMS</p><h2>Problems made <em>concrete.</em></h2></div><p>Case-study thinking over screenshot galleries: problem → system → engineering → outcome.</p></div><div className="work-grid">
        ...
        <section className="labs-section" id="labs"><div className="section-marker mono">06 / LABS</div><div className="labs-layout"><div><p className="eyebrow mono">AI / RAG / ENGINEERING</p><h2>Experiment, then<br /><em>make it useful.</em></h2></div><div className="lab-list"><div><span className="mono">01</span><strong>AI / RAG systems</strong><p>LLM applications, embeddings, retrieval, vector search, and practical AI workflows.</p></div><div><span className="mono">02</span><strong>Frontend architecture</strong><p>React, Angular, state, micro frontends, and design systems.</p></div><div><span className="mono">03</span><strong>Cloud / infrastructure</strong><p>Azure, Terraform, CI/CD, and systems that can be operated.</p></div></div></div></section>

        <section className="about-v2" id="about"><div className="section-marker mono">07 / AUTHOR</div><div className="author-layout"><div className="author-photo"><img src="/assets/profile.webp" alt="Portrait of Mohammad Moin, independent technology consultant and corporate technology trainer" /></div><div><p className="eyebrow mono">MOHAMMAD MOIN</p><h2>Build the system.<br />Develop the <em>capability.</em></h2><p>I work at the intersection of engineering, people, and practical delivery. I care about clear systems, useful abstractions, production reality, and helping teams move from knowing to doing.</p><p>The measure is simple: the solution works, the people can operate it, and the organization can keep moving.</p></div></div></section>

        <section className="contact-v2" id="contact"><div className="section-marker mono">08 / CONTACT</div><div className="contact-center"><p className="eyebrow mono">INDEPENDENT CONSULTANT · CORPORATE TECHNOLOGY TRAINER</p><h2>Have a problem worth<br /><em>building around?</em></h2><p>Consulting, corporate training, engineering work, or a conversation about what comes next.</p><a className="contact-link" href="mailto:mohammad.nicoll@gmail.com">mohammad.nicoll@gmail.com <Arrow /></a><div className="contact-meta mono"><span>BUILD PEOPLE</span><span>SOLVE PROBLEMS</span><span>INNOVATE</span></div></div></section>
      </main>
