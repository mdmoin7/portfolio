          ...
          {words.map((word, i) => {
            ...
            return (
              <motion.div
                ...
                style={{
                  opacity: useTransform(scrollYProgress, [Math.max(0, start - 0.18), start, end, Math.min(1, end + 0.18)], [0.12, 1, 1, 0.12]),
                  scale: useTransform(scrollYProgress, [Math.max(0, start - 0.18), start, end, Math.min(1, end + 0.18)], [0.72, 1, 1, 0.72]),
                  y: useTransform(scrollYProgress, [Math.max(0, start - 0.18), start, end, Math.min(1, end + 0.18)], [70, 0, 0, -70]),
                }}
              >
                {word}
              </motion.div>
...
function HeroVariant({ id }: { id: Variant }) {
    ...
    return (
      <section className="hero-lab-variant hero-lab-current">
        <div className="hero-lab-copy">
          <span className="hero-lab-eyebrow">WHO I AM · BASELINE</span>
          <h1>Mohammad Moin.</h1>
          <p>Build People. Solve Problems. Innovate.</p>
        </div>
  ...
  if (id === "B") {
    return (
      <section className="hero-lab-variant hero-lab-problem">
        ...
        <div className="hero-lab-choice-grid">
          {["BUILD", "TRANSFORM", "DEVELOP"].map((x) => (
            <motion.div key={x} whileHover={{ y: -8, scale: 1.02 }} className="hero-lab-choice">
              <span>0{x === "BUILD" ? "1" : x === "TRANSFORM" ? "2" : "3"}</span><strong>{x}</strong>
            </motion.div>
          ))}
        </div>
  ...
  if (id === "E") {
    return (
      <section className="hero-lab-variant hero-lab-blueprint">
        <div className="blueprint-corner">SYSTEM / 01<br />MOIN METHOD</div>
        <div className="blueprint-lines" />
        <div className="blueprint-core">
          <span>PEOPLE</span><i>↓</i><span>PROBLEM</span><i>↓</i><span>DESIGN</span><i>↓</i><span>BUILD</span><i>↓</i><strong>IMPACT</strong>
        </div>
  ...
  if (id === "F") {
    return (
      <section className="hero-lab-variant hero-lab-lightfield">
        <div className="lightfield-orb" />
        <div className="lightfield-copy">
          <span>ENGINEERING × PEOPLE × AI</span>
          <h1>Ideas become<br /><em>systems.</em></h1>
          <p>Scroll into the work.</p>
        </div>
    ...
    return (
      <section className="hero-lab-variant hero-lab-graph">
        ...
        <div className="graph-lines">
          {graphNodes.slice(0, -1).map(([name, x, y]) => <i key={name} style={{ left: x, top: y }} />)}
        </div>
        {graphNodes.map(([name, x, y]) => (
          <motion.div key={name} className={`graph-node ${name === "MOIN" ? "is-core" : ""}`} style={{ left: x, top: y }} whileHover={{ scale: 1.12 }}>
            {name}
          </motion.div>
        ))}
  ...
  if (id === "H") {
    return (
      <section className="hero-lab-variant hero-lab-scroll-narrative">
        <div className="hero-lab-narrative-intro">
          <span className="hero-lab-eyebrow">A SYSTEM IS MORE THAN ITS PARTS</span>
          <h1>What makes a<br /><em>good system?</em></h1>
        </div>
        <ScrollWords words={["TECHNOLOGY", "ARCHITECTURE", "CODE", "PEOPLE", "IT'S ALL OF THEM."]} />
        <div className="hero-lab-narrative-end">
          <span>PEOPLE × ENGINEERING × AI</span>
  ...
  return (
    <section className="hero-lab-variant hero-lab-system">
      <div className="hero-lab-system-copy">
        <span className="hero-lab-eyebrow">THE MOIN SYSTEM</span>
        <h1>How I<br /><em>work.</em></h1>
        <p>Scroll to assemble the system.</p>
      </div>
  ...
  return (
    <main className="hero-lab">
      <header className="hero-lab-header">
        <div>
          <span className="hero-lab-kicker">MOHAMMAD MOIN / HERO EXPLORATION</span>
          <h1>Which introduction<br /><em>makes you scroll?</em></h1>
        </div>
        <p>Interactive prototypes. Same identity, radically different opening experiences.</p>
      </header>

      <nav className="hero-lab-selector" aria-label="Hero concepts">
        {variants.map((x) => (
          <button key={x.id} className={active === x.id ? "is-active" : ""} onClick={() => setActive(x.id)}>
            <b>{x.id}</b><span>{x.name}</span>
          </button>
        ))}
      </nav>

      <div className="hero-lab-title">
        <span>OPTION {current.id}</span>
        <h2>{current.name}</h2>
      ...
      <footer className="hero-lab-footer">
        <span>HERO LAB</span>
        <p>Prototype only · reimagine/v2 · vercel-deploy remains untouched</p>
      </footer>
    </main>
  );
}
